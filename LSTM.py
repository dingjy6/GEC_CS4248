...
class Encoder(tf.keras.layers.Layer):
    ...
    def __init__(self,inp_vocab_size,embedding_size,lstm_size,input_length):
        ...
        self.embedding = Embedding(input_dim=inp_vocab_size, output_dim=embedding_size, input_shape=(input_length,),
    ...
    def call(self,input_sequence,states):
        ...
        #print(f"Encoder embedding output shape: {output1.shape}") 
        enco_output, enco_state_h, enco_state_c = self.encoder_lstm(output1, initial_state=states)
        #print(f"Encoder LSTM output shape: {enco_output.shape}") 
    ...
    def initialize_states(self,batch_size):
        ...
        #print(f"Initial states shape: {[initial_hidden_state.shape, initial_cell_state.shape]}")  
...
class Decoder(tf.keras.layers.Layer):
    ...
    def __init__(self,out_vocab_size,embedding_size,lstm_size,input_length):
        ...
        super().__init__()
        self.lstm_size = lstm_size
        self.embedding = Embedding(input_dim=out_vocab_size, output_dim=embedding_size,input_shape=(input_length,),
                           mask_zero=True,name="embedding_layer_encoder")
        self.lstmcell = tf.keras.layers.LSTMCell(lstm_size)
    ...
    def call(self,target_sequence,initial_states):
        ...
        #print(f"Decoder embedding output shape: {output2.shape}") 
        deco_output, deco_state_h, deco_state_c = self.decoder_lstm(output2, initial_state=initial_states)
        #print(f"Decoder LSTM output shape: {deco_output.shape}") 
...
class Encoder_decoder(tf.keras.Model):
    ...
    def __init__(self,inp_vocab_size,out_vocab_size,embedding_size,lstm_size,input_length,*args):
    ...
    def call(self,data,*args):
        ...
        initial_states = self.encoder.initialize_states(self.batch)
        encoder_output,encoder_final_state_h,encoder_final_state_c = self.encoder(input,initial_states)
...
model.fit(x=[train_enc_inp_truncated,train_dec_inp_truncated],y=train_dec_out_truncated,validation_data=([val_enc_inp_truncated,val_dec_inp_truncated],val_dec_out_truncated),epochs=45,batch_size=batch_size,callbacks=[tensorboard_callback,reducelr])
...
model.save_weights('enco_dec_lstm_wordbase.weights.h5')

# relaod saved model
model = Encoder_decoder(inp_vocab_size,out_vocab_size,embedding_dim,lstm_size,input_length)
optimizer = tf.keras.optimizers.Adam()
model.compile(optimizer=optimizer,loss=loss_function)
model.train_on_batch([incorr_train[:797],corr_train_inp[:797]],corr_train_out[:797])
model.load_weights('enco_dec_lstm_wordbase.weights.h5')
"""
...
cor_file_path = "/kaggle/input/output-text/cor.txt"  
ref_file_path = "/kaggle/input/output-text/ref.txt"
